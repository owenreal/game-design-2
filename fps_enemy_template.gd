...
func is_player_in_sight(player):
	var from_pos = self.global_transform.origin # ray starting point
	var to_pos = player.global_transform.origin # ray target point
	...
	ray_query.exclude = [get_rid()] # avoid self collision with ray
...
func _on_timer_timeout() -> void:
	nav_agent.target_position = Vector3.ZERO # or random or own position
